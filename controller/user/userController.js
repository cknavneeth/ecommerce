...
exports.signupRedirect=async(req,res)=>{
    try {
        ...
        const{username,email,password,password2}=req.body
        if(password!==password2){
            return res.render("user/signup",{error:"PASSWORD NOT MATCHING"})
        }
        const findUser=await User.findOne({email:email})
        console.log("kjhjkuj",email)
        if(findUser){
            return res.render("user/signup",{error:"EMAIL ALREADY EXISTS"})
        }

        const otp=generateOtp()
        console.log("makkale",email)
...
exports.otppage = async (req, res) => {
    try {
        ...
        console.error("Error", error);
        return res.status(500).json({ success: false, error: "Error occurred during OTP verification" });
    }
};
...
exports.resendOtp=async(req,res)=>{
     try {
        ...
        if(!email){
            res.status(400).json({success:false,error:"Email is not found.Sorry"})
        }
        const otp=generateOtp();
        req.session.userOtp=otp
        const emailSent=await sendVerificationEmail(email,otp)
    ...
    try {
      ...
      if (user) {
        ...
        if (isMatch) {
          ...
          return res.render('user/index'); // Render index page if login is successful
        } else {
